...
public class G1Pool
{
    ...
    public G1Pool(string name, GameObject go, int count)
    {
        this.name = name;
        this.go = go;
        this.count = count;
...
public class G1ObjectPool : MonoBehaviour
{
    public static G1ObjectPool Instance;

    //khai bao pool
    
    public Dictionary<int, int> dicClones = new Dictionary<int, int>();
    public List<G1Pool> pools = new List<G1Pool>();
    ...
    public G1Pool TryAddPoolByScript(G1Pool p)
    {
    ...
    public GameObject Get(G1Pool p, bool active = true)
    {
    ...
    public G1Pool CheckAddPool(GameObject obj)
    {
        foreach (var p in pools)
        {
            if (p.go == obj)
        ...
        var pool = new G1Pool(obj.name, obj, 0);
