...
public class G3_Pool
{
    ...
    public G3_Pool(string name, GameObject go, int count)
    {
        this.name = name;
        this.go = go;
        this.count = count;
...
public class G3_ObjectPool : MonoBehaviour
{
    public static G3_ObjectPool Instance;

    //khai bao pool
    public G3_Pool body;
    public G3_Pool smoke;
    public G3_Pool handdie;
    
    public Dictionary<int, int> dicClones = new Dictionary<int, int>();
    public List<G3_Pool> pools = new List<G3_Pool>();
    ...
    public G3_Pool TryAddPoolByScript(G3_Pool p)
    {
    ...
    public GameObject Get(G3_Pool p, bool active = true)
    {
    ...
    public G3_Pool CheckAddPool(GameObject obj)
    {
        foreach (var p in pools)
        {
            if (p.go == obj)
        ...
        var pool = new G3_Pool(obj.name, obj, 0);
